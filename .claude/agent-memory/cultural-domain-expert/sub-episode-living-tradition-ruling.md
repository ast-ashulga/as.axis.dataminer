...
- **`discus` → RENAMED to `iron-throw` (IMPLEMENTED 2026-06-27).** Murray 23.826 renders the sixth event "a mass of rough-cast iron" (Gk σόλος); prize IS the iron lump ("five revolving years to serve his need"). NOT the classical δίσκος — "discus" is an anachronism. Now `iron-throw` in `rules/segmentation/iliad.yaml`.
...
- **(b) Required signs (IMPLEMENTED 2026-06-27):** `living_tradition: true` now propagates from `cultural_sensitivity.living_tradition` in rules/segmentation/mahabharata.yaml into the per-tradition export manifest (via `sisyphus/phases/export.py`). Meridian still needs to read and surface this field. Flag must propagate export-manifest → Meridian → API → a PERSISTENT visible marker on every surfaced MB unit: fragment page AND any constellation node containing MB members (a user can reach MB via a cross-tradition node with no fragment page) AND search/share surfaces. Data flag DRIVES the badge; badge carries a short explanatory note.
